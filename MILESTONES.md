...
[Streamlit minimum viable dashboard](https://ds401-dern-volatility-dashboard.streamlit.app/)
Our dashboard allows the user to generate a 'realized' volatility surface for a selected date and stock ticker. The generated surface effectively creates a 'volatility skew' plot at our different time windows, then connects the lines to create a surface. We are currently in an R+D process to improve the surface generation technique. The other main modifications to the dashboard will be the ability to use different modeling techniques for surface generation, the ability to compare against implied volatility surfaces or points, and user interface tweaks for appearance and readability purposes.

Currently the user interface includes a dropdown (with search) to select from our over 3000 stock tickers, and a date entry (with calendar) to select a particular day to base surface generation off of. This triggers methods which find the correct parquet file to read data from, read in the needed data for the selected stock, perform our transformation and cleaning process, then plot based on our realized '_future' column values. Next steps include adding more plots than just our realized volatility surface, as we want our LGBM model result volatility surface and options market derived implied volatility surface visualizations to be added. We want to add some more controls related to different plots (Realized Volatility Surface, Predicted Volatility Surface, Options Implied Volatility Surface), and might add other visualizations to our web interface down the line.

## 2025-04-13: Add Models to Dashboard.

[Streamlit dashboard with models](https://ds401-dern-volatility-dashboard.streamlit.app/)
Our model attempts to fit a series of parameters which are used to generate our surface. The surface utilizes nine different parameters which are derived at six different time spans. Yang-Zhang Volatility, a proxy for Volatility Skew, a proxy for Volatility Curvature, a proxy for Wing Ratio, a proxy for Mean Reversion, a proxy for Volatility of Volatility, a proxy for Price Volatility Correlation, a proxy for Volatility Intensity, and the Log of Price ratio (current price/price n days ago) for 10, 15, 20, 25, 30, and 35 days out. These parameters are then 'pushed' n days forward, and we use these '_future' columns to generate the 'realized' surface. Our surface uses a poorly fitted ad-hoc SVI interpretation which we are actively working on improving. The model itself is an LGBM model which trains using a host of explanatory variables, listed below, which aim to fit to our nine '_future' parameters, meaning we effectively create 54 different models. We are still in the process of exploring if a) our parameterizations and proxies of traditional SVI and Heston parameters are even able to create a useful 'local' realized volatility surface (in the literature, local models are often contrasted with the SVI style approach we are currently taking, we ultimately want to follow the interpretation of a 'local' volatility surface), and b) if we might be able to do better than an LGBM model, but with the current approach and the scale of our data LGBM has been performing the best (when contrasted with simpler approaches like GARCH, complex models like xgboost, and deep learning strategies with LSTM layers).

Explanatory variables: open, high, low, close, volume, log returns, On-Balance Volume, Simple Moving Average, Exponential Moving Average, standard deviation, Average True Range, Donchian range, Parkinson volatility, volatility trend, intraday to full volatility, Simple/Exponential Moving Average ratio, Relative Strength Index, Chaikin Money Flow, Commodity Channel Index, On-Balance Volume change, Volume Weighted Average Price, VWAP deviation, Yang-Zhang volatility, quantile spread extreme, quantile spread interquartile range, skewness, kurtosis, volatility skew, volatility curvature, wing ratio, mean reversion, volatility of volatility, price-volatility correlation, volatility intensity, log price ratio, some evaluated with 10, 20, and 35 day windows and others with 10, 15, 20, 25, 30, 35 day windows.
